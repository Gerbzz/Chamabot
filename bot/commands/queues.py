...
async def global_queue_embed(interaction: nextcord.Interaction, queue_name: str):
	"""Create a global queue embed that works in any channel"""
	try:
		# Parse queue name and channel from the autocomplete result
		if ' (#' in queue_name:
			queue_name, channel_name = queue_name.split(' (#', 1)
			channel_name = channel_name.rstrip(')')
		
		# Search all queue channels for the specified queue
		target_queue = None
			for qc in bot.queue_channels.values():
			if queue := find(qc.queues, name=queue_name):
				target_queue = queue
					break

		if not target_queue:
			return await interaction.send("Queue not found in any channel", ephemeral=True)
		
		# Rest of the embed creation logic...
		# Remove any existing "not in queue channel" checks
		# ...
		# ...
		# ...
		# ...
